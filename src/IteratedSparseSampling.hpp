...
public:
	...
	IteratedSparseSampling(MPI_Comm communicator, int color, int group_size, InputIterator & input, int32_t seed_with_offset, unsigned int target_size) :
	 ...
	 */
	void loadSlice() {
		unsigned slice_portion = input_.edgeCount() / group_size_;
		unsigned slice_from = slice_portion * rank();
		// The last node takes any leftover edges
		unsigned slice_to = last() ? input_.edgeCount() : slice_portion * (rank() + 1);

		InputIterator::Iterator iterator = input_.begin();
		while (!iterator.end_) {
			if (iterator.position() >= slice_from && iterator.position() < slice_to) {
				edges_slice_.push_back(*iterator);
			}
			++iterator;
		}
	...
	bool initiateSampling(std::vector<AdjacencyListGraph::Weight> const & weights) {
		 ...
		 */
		unsigned number_of_edges_to_sample = (unsigned) std::pow((float) input_.vertexCount(), 1 + epsilon_ / 2);
