...
namespace mincut {
    ...
    void parallel_sample_edges(MPI_Comm comm, graph_slice<long> * graph, sitmo::prng_engine * random_generator, edge_struct_t * edge_sample) {
        ...
        MPI_Gather(&sum, 1, MPI_LONG, sums, 1, MPI_LONG, 0, comm);
        ...
        MPI_Scatter(edges_per_processor, 1, MPI_INT, &number_of_edges_to_select, 1, MPI_INT, 0, comm);
        ...
        MPI_Gatherv(rank ? selected_edges : MPI_IN_PLACE, number_of_edges_to_select, MPI_2INT, selected_edges, edges_per_processor, recv_displacements, MPI_2INT, 0, comm);
    ...
    int parallel_contract_try(MPI_Comm comm, graph_slice<long> * graph, sitmo::prng_engine * random_generator, int target_v) {
        ...
        MPI_Bcast(relabeling, virtual_v+1, MPI_INT, 0, comm);
    ...
    void distributed_matrix_transpose(long * src, long * dest, int k, int v, MPI_Comm comm) {
        ...
        MPI_Alltoall(src, k*k, MPI_LONG, dest, k*k, MPI_LONG, comm);
