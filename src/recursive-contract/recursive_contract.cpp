...
namespace mincut {
    ...
    long cut_reduce(MPI_Comm comm, long local_cut, int root) {
        ...
        MPI_Reduce(&local_cut, &cut_result, 1, MPI_LONG, MPI_MIN, root, comm); //This overwrites cut with the minimum value just in p0.
        return cut_result;
    }
...
//    	int counter = 0;
	...
	Graph duplicate_graph(MPI_Comm comm, int p, Graph *pGraph, MPI_Comm *newComm) {
		...
		if(rank < p) { //Sender processors.
			...
			MPI_Isend(&w, 1, MPI_INT, dest, INFOTAG1, comm, &req1); //Send w to the second group.
			MPI_Isend(&nVertices, 1, MPI_INT, dest, INFOTAG2, comm, &req2); //Send n to the second group.

			MPI_Isend(graph.begin(), w*n, type, dest, SLICETAG, comm, &req3); //Send the slice to the assigned second group processor.
			...
			MPI_Irecv(&w, 1, MPI_INT, source, INFOTAG1, comm, &req1);
			MPI_Irecv(&nVertices, 1, MPI_INT, source, INFOTAG2, comm, &req2);

			MPI_Wait(&req1, MPI_STATUS_IGNORE); //We need w to proceed, now.
			n = w*p;

			long *slice_memory = new long[w*n];
			nElements = n*w;

			MPI_Irecv(slice_memory, nElements, type, source, SLICETAG, comm, &req3);
			MPI_Wait(&req3, MPI_STATUS_IGNORE);

			MPI_Wait(&req2, MPI_STATUS_IGNORE); //We need nVertices to proceed, now.
	...
	void reassign_graph(MPI_Comm comm, int p2, Graph *pGraph, int x, MPI_Comm *newComm) {
		...
		if(rank < nSenders) { //Senders.
			...
			if(nRows < w && (dest + 1) < p2) {//Send to two processes.
				...
				MPI_Type_vector(nRows, n2, n, oldType, &blockType); //Re-define the type for the partially unpadded row.
				MPI_Type_commit(&blockType);
				MPI_Ibsend(toSend, 1, blockType, dest, SLICETAG, comm, &request1);
				MPI_Type_free(&blockType);
				...
				MPI_Type_vector(nRows, n2, n, oldType, &blockType); //Re-define the type for the partially unpadded row.
				MPI_Type_commit(&blockType);
				MPI_Ibsend(toSend, 1, blockType, dest, SLICETAG, comm, &request2);
				MPI_Type_free(&blockType);
			}
		...
		if(rank < p2) { //Receivers.
			...
			for(int i = 0; i < size && source < nSenders; i++) {
				if(nRows <= w && i != 0) {
					MPI_Irecv(toReceive, nRows*n2, oldType, source, SLICETAG, comm, request + i);
					break;
				}
				else {
			...
			if(secondSending) MPI_Wait(&request2, MPI_STATUS_IGNORE);

			//Wait for data to be arrived.
			for(int z = 0; z < size; z++) {
				if(reception[z]) MPI_Wait(request + z, MPI_STATUS_IGNORE);
				else break;
			}
